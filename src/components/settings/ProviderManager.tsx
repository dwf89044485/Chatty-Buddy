...
export function ProviderManager() {
  ...
  const sorted = [...providers]
    .filter((p) => !(
      p.provider_type === 'codebuddy-sdk' ||
      p.provider_type === 'codebuddy-cli' ||
      p.protocol === 'codebuddy-sdk' ||
      p.protocol === 'codebuddy-cli'
    ))
    ...
    <div className="space-y-6">
      ...
      {!loading && (
        <div className="rounded-lg border border-border/50 p-4 space-y-2">
          ...
          {/* Built-in zero-config providers */}

          <div className="border-b border-border/30 pb-2">
            <div className="flex items-center gap-3 py-2.5 px-1">
              <div className="shrink-0 w-[22px] flex justify-center">
                {getProviderIcon('CodeBuddy SDK', '')}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
            ...
            <p className="text-[11px] text-muted-foreground ml-[34px] leading-relaxed">
              {isZh ? '零配置，可在聊天页模型下拉中直接选择使用。' : 'Zero config. Select it directly from the chat model dropdown.'}
            </p>
          </div>
